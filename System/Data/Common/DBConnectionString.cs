...
internal sealed class DBConnectionString
{
	...
	private readonly System.Data.Common.NameValuePair _keychain;
	...
	internal System.Data.Common.NameValuePair KeyChain => _keychain;
	...
	private DBConnectionString(System.Data.Common.DbConnectionOptions connectionOptions, string restrictions, KeyRestrictionBehavior behavior, Dictionary<string, string> synonyms, bool mustCloneDictionary)
	{
		if ((uint)behavior <= 1u)
		{
			_behavior = behavior;
	...
	private bool IsRestrictedKeyword(string key)
	{
		...
		{
			return 0 > Array.BinarySearch(_restrictionValues, key, (IComparer<string>?)StringComparer.Ordinal);
		}
		return true;
	}
	...
	internal bool IsSupersetOf(DBConnectionString entry)
	{
		...
		{
			for (System.Data.Common.NameValuePair nameValuePair = entry.KeyChain; nameValuePair != null; nameValuePair = nameValuePair.Next)
			{
	...
	private static string[] NewRestrictionAllowOnly(string[] allowonly, string[] preventusage)
	{
		...
		{
			if (0 > Array.BinarySearch(preventusage, allowonly[i], (IComparer<string>?)StringComparer.Ordinal))
			{
				if (list == null)
				{
	...
	private static string[] NewRestrictionIntersect(string[] a, string[] b)
	{
		...
		{
			if (0 <= Array.BinarySearch(b, a[i], (IComparer<string>?)StringComparer.Ordinal))
			{
				if (list == null)
				{
					list = new List<string>();
	...
	private static string[] NoDuplicateUnion(string[] a, string[] b)
	{
		...
		{
			if (0 > Array.BinarySearch(a, b[j], (IComparer<string>?)StringComparer.Ordinal))
			{
				list.Add(b[j]);
			}
		}
	...
	internal static string[] RemoveDuplicates(string[] restrictions)
	{
		...
		if (0 < num)
		{
			Array.Sort(restrictions, (IComparer<string>?)StringComparer.Ordinal);
